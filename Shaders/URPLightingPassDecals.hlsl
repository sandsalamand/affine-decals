...
#include "URPMacrosDecal.hlsl"
#include "DecalLitInput.hlsl"

void InitializeInputData(GeometryOut input, half3 normalTS, out InputData inputData)
{
...
GeometryOut LitPassVertex(Attributes input)
{
	...
	UPDATE_ATTRIBUTES_TO_VARYINGS(input, output);

    return output;
}

// Used in Standard (Physically Based) shader
half4 LitPassFragment(GeometryOut input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
