...
#define DECAL_TEX_MAX_WIDTH 256

// NOTE: Do not ifdef the properties here as SRP batcher can not handle different layouts.
CBUFFER_START(UnityPerMaterial)
float4 _BulletDecalPositions[MAX_DECALS]; //TODO: use the 4th float to hold an index to the decal to draw so that we can reuse this for all decals
float4 _BulletDecalNormals[MAX_DECALS];
float4 _BulletDecalTangents[MAX_DECALS];
float _BulletMapWidth;
float _BulletMapHeight;
float _BulletMapDepth;
float4 _DecalAlbedo_TexelSize;
float4 _DecalNormal_TexelSize;
half _TextureResolution;
...
TEXTURE2D_ARRAY(_DecalAlbedoArray);         SAMPLER(sampler_point_clamp_DecalAlbedoArray);
TEXTURE3D(_BulletMap);                      SAMPLER(sampler_BulletMap);

TEXTURE2D(_DecalAlbedo);                    SAMPLER(sampler_point_clamp_DecalAlbedo);
TEXTURE2D(_DecalNormal);                    SAMPLER(sampler_point_clamp_DecalNormal);
